...
for s in KKI_0050815; do #$(cat /home/despoB/kaihwang/bin/ThaDev/list_of_controls)
	...
	if [ -e ${WD}/func_preproc/${s}_func_preproc.nii.gz ]; then
		for roi in Gordon_333_3mm Morel_3mm; do #Gordon_333_3mm WTA_3mm
			3dNetCorr \
		...
		#Gordon_plus_WTA_3mm.nii.gz
		3dNetCorr \
		-inset ${WD}/func_preproc/${s}_func_preproc.nii.gz \
		-in_rois /home/despoB/kaihwang/Rest/ABIDE/ROIs/Gordon_plus_Morel_3mm.nii.gz \
		-prefix /tmp/KH_${s}/Adj_Gordon_plus_Morel_3mm

		num=$(expr $(wc -l /tmp/KH_${s}/Adj_Gordon_plus_Morel_3mm_000.netcc | awk '{print $1}') - 6)
		tail -n $num /tmp/KH_${s}/Adj_Gordon_plus_Morel_3mm_000.netcc > /home/despoB/connectome-thalamus/NotBackedUp/ParMatrices/${s}_Gordon_plus_Morel_3mm_corrmat  
